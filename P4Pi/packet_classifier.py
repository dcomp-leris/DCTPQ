...
# 'IPI', 'FS', 'PS'
    ...
    def make_prediction(self, features, flow_key):
        try:
            ...
            # model_input = np.array([[ifi_average, ipi_average, fs_average]])       # This is for RTP Layer Features
            model_input = np.array([[ipi_average, fs_average, ps_average]])
# This is for IP Layer Features
            #print(model_input)
            ...
            #print('=====================================================================================================')
            #return [prediction[0],(ifi_average,ipi_average,fs_average)] # For RTP Features
            return [prediction[0],(ipi_average,fs_average,ps_average)]
    ...
    def classify_packet(self, packet):
        ...
        if IP in packet:
            ...
            #flow_key = str((packet[IP].src, packet[IP].dst, packet[UDP].dport)).encode().hex() #self.hasher.hexdigest()
             ...
             flow_key = hashlib.sha256(str((packet[IPv6].src, packet[IPv6].dst, packet[UDP].dport)).encode().hex().encode()).hexdigest()
